...
class _AnalysisScreenState extends State<AnalysisScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadHoldings() async {
    ...
    try {
      ...
      // 2. 국내 보유종목 로드 (캐시 무시하고 API 직접 호출)
      print('🇰🇷 국내 보유종목 로드 중... (캐시 무시)');
      ...
      // 3. 해외 보유종목 로드 (캐시 무시하고 API 직접 호출)
      print('🌍 해외 보유종목 로드 중... (캐시 무시)');
      ...
      try {
        ...
        // NASD 거래소 조회
        print('🌍 NASD 거래소 해외 보유종목 조회...');
        final nasdBalance = await _unifiedApiService.getOverseasAccountBalance(
          cano: cano,
        ...
        if (nasdBalance != null && nasdBalance['holdings'] != null) {
          final nasdHoldings = nasdBalance['holdings'] as List;
          overseasHoldings.addAll(nasdHoldings.cast<Map<String, dynamic>>());
          print('✅ NASD 해외 보유종목: ${nasdHoldings.length}개');
        }
        
        // NYSE 거래소 조회
        print('🌍 NYSE 거래소 해외 보유종목 조회...');
        final nyseBalance = await _unifiedApiService.getOverseasAccountBalance(
          cano: cano,
          acntPrdtCd: acntPrdtCd,
          ovrsExcCd: 'NYSE',
          trCrcyCd: 'USD',
        ...
        if (nyseBalance != null && nyseBalance['holdings'] != null) {
          final nyseHoldings = nyseBalance['holdings'] as List;
          overseasHoldings.addAll(nyseHoldings.cast<Map<String, dynamic>>());
          print('✅ NYSE 해외 보유종목: ${nyseHoldings.length}개');
        }
        
        print('✅ 총 해외 보유종목: ${overseasHoldings.length}개');
        
        // 해외 보유종목 상세 디버깅
        for (int i = 0; i < overseasHoldings.length; i++) {
          final holding = overseasHoldings[i];
          print('🔍 해외 보유종목 $i: ${holding.keys.toList()}');
          print('  - 데이터: $holding');
          print('  - 종목코드: ${holding['pdno'] ?? holding['stockCode'] ?? 'N/A'}');
          print('  - 종목명: ${holding['prdt_name'] ?? holding['stockName'] ?? 'N/A'}');
          print('  - 수량: ${holding['ovrs_cblc_qty'] ?? holding['cblc_qty'] ?? holding['hldg_qty'] ?? holding['quantity'] ?? 'N/A'}');
        }
      } catch (e) {
      ...
      allHoldingsRaw.addAll(domesticHoldings);
      allHoldingsRaw.addAll(overseasHoldings);

      // 정규화: API마다 다른 키를 공통 키로 매핑하고 유효하지 않은 항목 제거
      print('🔧 보유종목 데이터 정규화 시작... (총 ${allHoldingsRaw.length}개)');
      final List<Map<String, dynamic>> allHoldings = allHoldingsRaw
          .map((item) {
            final String stockCode = (item['stockCode'] ?? item['stock_code'] ?? item['pdno'] ?? item['symbol'] ?? '').toString();
            final String stockName = (item['stockName'] ?? item['stock_name'] ?? item['prdt_name'] ?? item['name'] ?? '').toString();
            
            // 안전한 타입 변환 (문자열도 숫자로 변환 가능하도록)
            int quantity = 0;
            try {
              final qtyValue = item['quantity'] ?? item['hldg_qty'] ?? item['qty'] ?? item['ovrs_cblc_qty'] ?? item['cblc_qty'] ?? 0;
              if (qtyValue is num) {
            ...
            double avgPrice = 0.0;
            try {
              final priceValue = item['avgPrice'] ?? item['avg_prc'] ?? item['pchs_avg_pric'] ?? 0;
          ...
          .where((e) {
            // 해외 보유종목은 수량이 0이어도 표시 (보유 중인 종목이므로)
            final isValid = e.isNotEmpty && e['stockCode'] != null && e['stockCode'].toString().isNotEmpty;
            if (!isValid) {
              print('❌ 필터링 제외: ${e['stockCode']} - 수량: ${e['quantity']}');
            }
            return isValid;
          })
          .toList();
  ...
  Widget _buildHoldingsTab() {
    ...
    print('  - _isFirstLoading: $_isFirstLoading');
    print('  - _holdingsItems.length: ${_holdingsItems.length}');
    ...
    final hasApiConfig = KisUnifiedApiService().apiConfig['isValid'] as bool? ?? false;
    print('  - hasApiConfig: $hasApiConfig');
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return AnimatedSwitcher(
              ...
              child: Container(
                key: ValueKey('$stockCode-${analysis?['timestamp'] ?? DateTime.now().millisecondsSinceEpoch}'),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // 통합 분석 결과 (AI 분석 + 기술적 지표)
                if (analysisData != null) 
                  FutureBuilder<Widget>(
                    future: _buildIntegratedAnalysisSection(item, analysisData),
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
  ...
  Widget _buildStockHeader(dynamic item, Map<String, dynamic>? currentPriceData) {
    ...
    // 실시간 API 데이터에서 현재가/전일가 가져오기 (분석 결과보다 우선)
    final analysis = _analysisResults[stockCode];
    final currentPrice = currentPriceData?['currentPrice'] ?? analysis?['currentPrice'] ?? 0.0;
    final prevClose = currentPriceData?['prevClose'] ?? analysis?['prevClose'] ?? currentPrice;
    
    print('🔍 현재가 데이터 소스 확인:');
    print('  - API 데이터: ${currentPriceData?['currentPrice']}');
    print('  - 분석 결과: ${analysis?['currentPrice']}');
    print('  - 최종 현재가: $currentPrice');
  ...
  Widget _buildHoldingsInfo(Map<String, dynamic> holding, Map<String, dynamic>? analysis) {
    ...
    // 실시간 API 데이터 우선 사용: API 데이터가 있으면 무조건 사용
    if (currentPriceData != null && currentPriceData['currentPrice'] != null) {
      currentPrice = currentPriceData['currentPrice'].toDouble();
    }
    
    // 분석 결과의 currentPrice는 신뢰하지 않음 (잘못된 데이터 가능성)
    
    // 손익 계산 (현재가 - 평균단가) * 수량
    final profit = quantity > 0 && avgPrice > 0 && currentPrice > 0 
        ? (currentPrice - avgPrice) * quantity 
        : (holding['profit'] ?? holding['profitLoss'] ?? 0.0).toDouble();
    
    // 손익률 계산 (현재가 - 평균단가) / 평균단가 * 100
    final profitRate = avgPrice > 0 && currentPrice > 0 
        ? ((currentPrice - avgPrice) / avgPrice) * 100 
        : (holding['profitRate'] ?? holding['profit_rate'] ?? 0.0).toDouble();
    ...
    print('  - 계산된 profit: $profit');
    print('  - 계산된 profitRate: ${profitRate.toStringAsFixed(2)}%');
  ...
  Future<Widget> _buildIntegratedAnalysisSection(dynamic item, Map<String, dynamic> analysis) async {
    final confidence = analysis['confidence'] ?? 0.0;
    final targetPrice = analysis['targetPrice'] ?? 0.0;
    final signal = analysis['signal'] ?? '관망';
    final reason = analysis['reason'] ?? '';
              ...
              children: [
                ...
                if (targetPrice > 0)
                  Column(
                    ...
                    children: [
                      Text(
                        _isNasdaqStock(analysis['stockCode'] as String? ?? '')
                      ...
                      Text(
                        _isNasdaqStock(analysis['stockCode'] as String? ?? '')
                          ? '분석가: \$${analysisPrice.toStringAsFixed(2)}'
                          : '분석가: ${NumberFormat('#,###').format(analysisPrice)}원',
                        style: TextStyle(
                          fontSize: 11,
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.lightbulb_outline, size: 16, color: signalColor),
                ...
                const SizedBox(height: 8),
                Text(
                  reason.isNotEmpty ? reason : _generateComprehensiveAnalysisReason(analysis, finalSignal, comprehensiveScore, individualScores),
                  style: TextStyle(
                    fontSize: 13,
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.analytics, size: 16, color: signalColor),
                    const SizedBox(width: 6),
                    Text(
                      '기술적 지표 분석',
                      style: TextStyle(
                ...
                // 지표별 상세 정보
                ...individualScores.entries.map((entry) {
                  final indicatorName = entry.key;
                  // _getIndicatorScore 메서드를 통해 올바른 점수 가져오기
                  final score = _getIndicatorScore(indicatorName, analysis);
                  String signal = '관망';
                  ...
                  // 상세 분석 이유 생성
                  final detailedReason = _generateDetailedReason(indicatorName, score, signal, analysis);
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Row(
                                children: [
                                  Text(
                                    _getKoreanIndicatorName(indicatorName),
                                    style: TextStyle(
                                      fontSize: 13,
                                      fontWeight: FontWeight.w600,
                                      color: indicatorColor,
                                  ...
                                  Container(
                                    ...
                                    child: Text(
                                      '${score >= 0 ? '+' : ''}${score.toStringAsFixed(3)}',
                                      style: TextStyle(
                                        fontSize: 11,
                                        fontWeight: FontWeight.bold,
                                        color: indicatorColor,
  ...
  String _generateComprehensiveAnalysisReason(Map<String, dynamic> analysis, String signal, double aggregateScore, Map<String, dynamic> individualScores) {
    ...
    if (aggregateScore > 0.1) {
      reasons.add('종합 점수 ${aggregateScore.toStringAsFixed(2)}로 강한 매수 신호를 보입니다.');
    } else if (aggregateScore > 0.05) {
      reasons.add('종합 점수 ${aggregateScore.toStringAsFixed(2)}로 매수 신호를 보입니다.');
    } else if (aggregateScore < -0.1) {
      reasons.add('종합 점수 ${aggregateScore.toStringAsFixed(2)}로 강한 매도 신호를 보입니다.');
    } else if (aggregateScore < -0.05) {
      reasons.add('종합 점수 ${aggregateScore.toStringAsFixed(2)}로 매도 신호를 보입니다.');
    } else {
      reasons.add('종합 점수 ${aggregateScore.toStringAsFixed(2)}로 중립적인 상태입니다.');
    }
    ...
    individualScores.forEach((indicatorName, score) {
      final scoreValue = (score as num?)?.toDouble() ?? 0.0;
      if (scoreValue > 0.05) {
        buyIndicators.add(_getKoreanIndicatorName(indicatorName));
      } else if (scoreValue < -0.05) {
        sellIndicators.add(_getKoreanIndicatorName(indicatorName));
      }
    });
    
    if (buyIndicators.isNotEmpty) {
      final buyList = buyIndicators.join(', ');
      reasons.add('매수 신호 지표: $buyList');
    }
    
    if (sellIndicators.isNotEmpty) {
      final sellList = sellIndicators.join(', ');
      reasons.add('매도 신호 지표: $sellList');
    }
  ...
  /// 기술적 지표 조건 표시 (2열 배치, 매수/매도 색상 구분) - 기존 메서드 유지
